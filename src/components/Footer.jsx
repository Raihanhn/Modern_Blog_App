    ...
    <>
      <footer className="bg-gradient-to-b from-emerald-400 to-green-300 pt-24 pb-7 ">
        <div className="grid items-center grid-cols-3 justify-items-center ">
          ...
          {links?.map((list, i) =>(
            <ul className="grid items-center justify-items-center gap-1" key={i} >
              {list?.map((val, i) =>(
                <li key={i} className='font-medium text-sm sm:text-xs' >{val.link} </li>
              ))}
            </ul>
          ))}
        </div>
        <div className="w-7/12 lg:w-[95vw] m-auto mt-9">
          ...
          <div className="flex items-center justify-between px-7 md:px-0 md:gap-5 md:flex-col-reverse">
            <p className="text-sm md:text-center">Copyright <sup className="text-base font-bold">&copy;</sup>All Reserved Right 2023 | <span className='font-semibold' >DEVELOPER RAIHAN</span></p>
            <div className="flex items-center gap-3">
              {sociallinks?.map((val, i) =>(
                <img src={val.icon} key={i} alt="social/icons" className='w-5 h-5' />
              ))}
            </div>
          </div>
        </div>
